...
function updateAirQualityCloudGaugeDescription(aqi, locationName) {
  ...
  if (aqi <= 50) {
    ...
    aqiDescription.textContent = `The air quality index in ${locationName} is ${aqi} and it means that the air quality is good. Nevertheless we can still think about the ways to make it better!`;
  } else if (aqi <= 100) {
    clouds.style.filter = "invert(0) brightness(0.5) blur(9px)";
    aqiDescription.textContent = `The air quality index in ${locationName} is ${aqi} and it means that the air quality is moderate. Usually sensitive individuals should consider limiting prolonged outdoor exertion.`;
  } else if (aqi <= 150) {
    clouds.style.filter = "invert(0) brightness(0.3) blur(9px)";
    aqiDescription.textContent = `The air quality index in ${locationName} is ${aqi} and it means that the air quality is Unhealthy for Sensitive Groups. Children, active adults, and people with respiratory disease, such as asthma, should limit prolonged outdoor exertion.`;
  } else if (aqi <= 200) {
    clouds.style.filter = "invert(1) brightness(0)";
    aqiDescription.textContent = `The air quality index in ${locationName} is ${aqi} and it means that the air quality is Unhealthy. Children, active adults, and people with respiratory disease, such as asthma, should limit prolonged outdoor exertion.`;
  } else if (aqi <= 300) {
    clouds.style.filter = "invert(1) brightness(0)";
    aqiDescription.textContent = `The air quality index in ${locationName} is ${aqi} and it means that the air quality is Very Unhealthy. Children, active adults, and people with respiratory disease, such as asthma, should avoid outdoor exertion; everyone else should limit outdoor exertion.`;
  } else if (aqi <= 500) {
    clouds.style.filter = "invert(1) brightness(0)";
    aqiDescription.textContent = `The air quality index in ${locationName} is ${aqi} and it means that the air quality is Hazardous. Everyone should avoid all physical activity outdoors.`;
  }
}

////// TO DO LIST/ THOUGHTS LIST
